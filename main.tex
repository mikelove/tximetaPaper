...
\cite{refgenie}. Arkas, ARMOR, pepkit, and basejump are all
...
In 2015, Belhajjame \textit{et al.}~\cite{Belhajjame2015} introduced the concept of a ``Research
...
workflow. Belhajjame \textit{et al.}~\cite{Belhajjame2015} summarized literature in the field of
computational reproducibility and efforts toward extensive provenance
tracking. The developers of the Common Workflow Language (CWL) \cite{cwl}
...
cwltool \cite{Khan2018}. The developers of CWLProv emphasized the
...
sequences. Gruning \textit{et al.}~\cite{Gruning2018} recommended combining systems such as
...
\begin{tabular}{llccc} 
  ...
  \bf{Software} & \bf{Domain} & \bf{Ranges} & \bf{Release} & \bf{Post-hoc} \\
   ...
   & & \bf{attach.} & \bf{attach.} & \bf{possible} \\
  \thickhline
  \underline{tximeta} & RNA-seq import                & \checkmark & \checkmark & \checkmark \\
  tximport \cite{tximport} & RNA-seq import           & & & \\
  \hline
  htseq \cite{htseq} & RNA-seq count.                 & & & \\
  featureCounts \cite{featurecounts} & RNA-seq count. & \checkmark & & \\
  summarizeOverlaps \cite{granges} & RNA-seq count.   & \checkmark & & \\
  \hline
  Arkas \cite{arkas} & RNA-seq analysis               & \checkmark & & \\
  ARMOR \cite{Orjuelag2019} & RNA-seq analysis        & \checkmark & & \\
  \hline
  pepkit \cite{pepkit} & Workflow mngmt.              & \checkmark & & \\
  basejump \cite{basejump} & Utilities                & & & \\
  Refgenie \cite{refgenie} & Genome mngmt.            & & & \checkmark \\
  CRAM+RefGet \cite{cram,refget} & Genome alignment   & & & \checkmark \\
  CWLProv \cite{Khan2018} & Provenance tracing        & & & \checkmark \\
\hline
\end{tabular}
\begin{flushleft}
  \hl{Text text text text text text text (Abbreviations:)}
\end{flushleft}
\label{tab:comp}
\end{table}
...
\begin{table}[t]
  \centering
  \caption{\bf{Pre-computed reference transcripts checksums in tximeta as of Fall 2019.}}
\begin{tabular}{llll} 
  \hline
...
\texttt{rowRanges} slot.} The SummarizedExperiment object
can be passed to various downstream packages such as
DESeq2, edgeR, limma-voom, or fishpond, with example code in the
tximeta package vignette \cite{deseq2,edger,limma,voom,swish}. The
