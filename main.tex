...
\charlotte{What about adding a figure with the workflow, e.g. like the one in the tximeta GitHub repo README?}

\mike{great idea, I'll do this this week}

\peter{agreed, great idea!}
...
analysis with the same data and code) may be difficult or
impossible. Manually keeping track of these critical pieces of
metadata throughout a long-term bioinformatic project is tedious and
error prone; still manual metadata tracking is a common practice in
RNA-seq bioinformatics. For example, a common approach to tracking the
reference transcripts that were used during quantification would be to
keep a \texttt{README} file in the same directory as the
quantification files, with information about the provenance of the
...
analysis: if quantification files are shared with genomic coordinates
but without criticial metadata about the genome version, computation of
overlaps with other genomic data with mismatching genome versions can
...
describing the computational analysis of RNA-seq data, and contend
...
% talk here about VR and seqrepo:
% https://vr-spec.readthedocs.io/en/1.0rc/
...
transcriptome in the hash table then a non-ranged SummarizedExperiment \peter{minor: the ranges aren't taken away, so non-ranged sounds better to my ear than un-ranged}
...
about the reference transcriptome and pointers \peter{Different term? 'pointers' has the technical computer programming meaning which I don't think this is} to the source file(s)
...
\peter{Why is it `release XX' below and not an actual version number? Also, the other example output has a line after `read\_tsv' giving progress on how many files read. Super minor: GENCODE spell it all caps.}
\begin{verbatim}
...
\mike{need to figure out how to chop these long URLs...}
...
% should we talk here about the next challenge:
% when quantifying allelic expression, we will need
% to have stable identifiers for the alleles as well.
% there is some existing work here (also using hashed checksums)
% 'vr' + 'seqrepo'
% https://vr-spec.readthedocs.io/en/1.0rc/
% https://github.com/biocommons/biocommons.seqrepo
