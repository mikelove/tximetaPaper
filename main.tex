...
\begin{tabular}{llccc} 
  ...
  summarizeOverlaps \cite{granges} & RNA-seq count.   & \checkmark & \checkmark & \\
  \hline
...
\begin{flushleft}
  \hl{Tximeta is compared to related software, grouped by domain.
    ...
    post-hoc lookup of transcriptome-related metadata is possible from
    publicly shared data files or objects.
    ...
    *Note that ARMOR imports tximeta for building SummarizedExperiment
    objects with transcript ranges and release information
    automatically attached. (Abbreviations:)}
\end{flushleft}
\label{tab:comp}
\end{table}
