...
\title{Tximeta: reference sequence checksums for \\ provenance
  ...
  Soneson, Peter Hickey, Lisa K. Johnson, \\ Tessa Pierce, Lori
  Shephard, Martin Morgan, Rob Patro}
...
of sequences in FASTA format and transcript locations in a genome in
GTF/GFF (gene transfer format / general feature format). Alternatively
a novel reference may be prepared as part of the data analysis. The
provenance of the reference transcripts, their source and release
number, can be considered critical metadata with respect to the
processed data, the files with quantification information. Without
...
analysis with the same data and code) may be difficulty or
...
error prone, though manual metadata tracking is a common practice in
...
but without crticial metadata about the genome version, computation of
...
summarization of transcript data to gene level is made more difficult
...
the sequences themselves do not need to be stored in total within an
...
% talk here about seqrepo:
% https://github.com/biocommons/biocommons.seqrepo
% https://docs.google.com/presentation/d/1AN2hHstDeijPQ2uDRss9BafK6jWEwP-DCKD6FjGxyr0/edit#slide=id.p
