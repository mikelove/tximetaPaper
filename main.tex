...
RNA-seq sample quantification. \charlotte{This sentence sounds a bit strange to me. Perhaps something like "Tximeta is similar in implementation to the CRAM format in the use of hashed checksums, but identifies the transcript sequences used during sample quantification rather than the genome sequence used during alignment."}. We see tximeta as a piece of a larger
effort to create software systems that are ``more amenable to
reproducibility'' \citep{Peng2011}.

\charlotte{In the figure, the "quantification" arrow goes to the index, which then feeds further into the quant output. What about pointing the "quantification" arrow directly to the quant output, and put the "quantification" label over both that arrow and the one from the index to the quant output?}
...
For example, in the fishpond Bioconductor package vignette \charlotte{cite?}, the
...
identifiers \charlotte{Would it be pedagogical if the example contained the "files" column?}. It is expected that the quantification files are located
within the original directory structure created by Salmon and with all
the associated metadata files. One paradigm used in the fishpond vignette
...
\charlotte{One thing that just came to mind: the actual annotation columns (geneID etc) added to the SE by default depend on the reference source, right - is it worth mentioning what information will be added, or maybe it's too much detail (and may change). Maybe exemplify?}
